...
const INITIAL_STATE = {
    currentPage: 'access_code', // Default to access code
    unlockedRiddleChars: [],
    completedQuestions: [],
    riddleInputs: {},
...
export const GameProvider = ({ children }) => {
    const [gameState, setGameState] = useState(() => {
        ...
        if (saved) {
            try {
                ...
                return { ...state, currentPage: 'access_code', hasAccess: false };
            } catch (e) {
    ...
    const grantAccess = () => {
        setGameState(prev => ({
            ...prev,
            hasAccess: true,
            currentPage: prev.onboardingComplete ? 'menu' : 'language',
            direction: 'forward'
        }));
    };
    ...
    return (
        <GameContext.Provider value={{ gameState, setGameState, navigateTo, completeQuestion, updateRiddleInput, isQuestionCompleted, resetApp, setCurrentQuestion, submitAnswer, grantAccess, completeOnboarding, solveRiddle, unlockAllQuestions }}>
            {children}
